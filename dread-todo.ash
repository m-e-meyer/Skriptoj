...
choice_info[string][string][string][string] dread_state = {
	"Forest": {
		"cabin" : {
			"kitchen" : {
				...
				"flour mill" : new choice_info(9999, "[S #1b] get bone flour", "mus",
											   $items[old dry bone]),
				"disposal" : new choice_info(1, "[X] banish stench from forest", "", { })
			},
			"basement" : {
				"newspapers" : new choice_info(10, "[F] get freddies", "", { }),
				"diary" : new choice_info(9999, "[E #1a] get spooky dmg buff", "", { }),
				"lockbox" : new choice_info(1, "[G #4] get auditor's badge", "", $items[replica key]),
				"banana" : new choice_info(9999, "[G #2a] make lock impression", "",
										   $items[wax banana]),
			},
			"attic" : {
				"music box" : new choice_info(1, "[X] banish spooky from forest ([G #2b] AT gets parts)",
											  "", { }),
		...
		"tree" : {
			"climb" : {
				...
				"kick" : new choice_info(1, "[X] banish sleaze from forest", "", { }),
		...
		"burrows" : {
			"heat" : {
				"pull cork" : new choice_info(1, "[X] banish hot from forest", "", { }),
				"play" : new choice_info(9999, "[E #1b] get hot dmg buff", "", { }),
				"melt" : new choice_info(9999, "[C #1] get cool iron ingot", "",
										 $items[old ball and chain]),
			},
			"cold" : {
				"read" : new choice_info(1, "[X] banish cold from forest", "", { }),
	...
	"Village": {
		"square" : {
			...
			"blacksmith" : {
				"furnace" : new choice_info(1, "[X] banish cold from village", "", { }),
			...
			"gallows" : {
				"paint noose" : new choice_info(1, "[X] banish spooky from village", "", { }),
		...
		"skid row" : {
			"sewers" : {
				"unclog" : new choice_info(1, "[X] banish stench from village", "", { }),
				"slosh" : new choice_info(9999, "[E #1c] get stench dmg buff", "", { }),
			},
			"tenements" : {
				"rats" : new choice_info(1, "fewer skeletons in castle", "", { }),
				"paint" : new choice_info(1, "[X] banish sleaze from village", "", { }),
		...
		"estate" : {
			"plot" : {
				...
				"tombstones" : new choice_info(9999, "[E #1d] get sleaze dmg buff", "", { }),
			},
			"quarters" : {
				"ovens" : new choice_info(1, "[X] banish hot from village", "", { }),
	...
	"Castle": {
		"tower" : {
			...
			"bedroom" : {
				"parrot" : new choice_info(1, "[X] banish sleaze from castle", "", { }),
		...
		"hall" : {
			...
			"kitchen" : {
				"freezer" : new choice_info(1, "[X] banish cold from castle", "", { }),
				"hang out" : new choice_info(9999, "[E #1e] get cold dmg buff", "", { }),
			},
			"dining room" : {
				"roast" : new choice_info(1, "[S #1a] get dreadful roast", "", { }),
				"dishes" : new choice_info(1, "[X] banish stench from castle", "", { }),
				"levitate" : new choice_info(1, "[G #1] get wax banana", "mys", { }),
			},
		},
		"dungeons" : {
			"cell block" : {
				"flush" : new choice_info(1, "[X] banish spooky from castle", "", { }),
				"pushups" : new choice_info(9999, "gain muscle", "", { }),
				"nap" : new choice_info(9999, "gain mp", "", { }),
			},
			"boiler room" : {
				"steam" : new choice_info(1, "[X] banish hot from castle", "", { }),
...
void main(string... d_args)
{
	string args = join_strings(d_args, '');
	acli_command_parameters acp = acli_parse(args, "bcfgstvwxz?");
	...
	matcher logmatch = create_matcher("(Dreadsylvania:.*?)Loot Distribution:", buf);
	if (! find(logmatch)) {
	...
	print(`{1000-zone_defeats['Forest']} remaining in Forest`);
	print(`{1000-zone_defeats['Village']} remaining in Village`);
	print(`{1000-zone_defeats['Castle']} remaining in Castle`);
	...
	if (quests_selected["[T"]) {
		print("");
		print("Consumable trophies progress:");
		buffer b = visit_url("showconsumption.php");
		matcher m = create_matcher(">([^<]*)</a>[^<]*</td>[^<]*<td>([0-9,]*)", b);
		int bw_consumed = 0;
		int ka_consumed = 0;
		while (find(m)) {
			string c = group(m, 1);
			if (c == "electric Kool-Aid")
				ka_consumed = group(m, 2).to_int();
			else if (c == "bottle of Bloodweiser")
				bw_consumed = group(m, 2).to_int();
			if (bw_consumed > 0 && ka_consumed > 0)
				break;
		}
		item bw = $item[bottle of bloodweiser];
		int bw_bag = item_amount(bw);
		int bw_closet = closet_amount(bw);
		print(`* Bloodweiser in bag: {item_amount(bw)}`);
		print(`* Bloodweiser in closet: {closet_amount(bw)}`);
		print(`* Bloodweiser consumed: {bw_consumed}`);
		if (bw_bag + bw_closet + bw_consumed < 50)
			print(`Need {50 - bw_bag - bw_closet - bw_consumed} more Bloodweisers`, "red");
		else
			print(`Have enough Bloodweisers to proceed!`, "green");
		item ka = $item[electric kool-aid];
		int ka_bag = item_amount(ka);
		int ka_closet = closet_amount(ka);
		print(`* Kool-Aid in bag: {item_amount(ka)}`);
		print(`* Kool-Aid in closet: {closet_amount(ka)}`);
		print(`* Kool-Aid consumed: {ka_consumed}`);
		if (ka_bag + ka_closet + ka_consumed < 50)
			print(`Need {50 - ka_bag - ka_closet - ka_consumed} more Kool-Aids`, "red");
		else
			print(`Have enough Kool-Aids to proceed!`, "green");
	}
}
